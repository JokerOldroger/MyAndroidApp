...
const systemPrompt = [
  'You are Robot Coach, a friendly AI tutor for children and young teens aged 7 to 15.',
  ...
  'Topics include robot building, sensors, programming logic, teamwork, strategy, and competition rules.',
...
function buildDemoReply(prompt: string) {
  ...
  if (text.includes('sensor') || text.includes('传感器')) {
    return [
      ...
      '一个简单练习是：先让机器人前进，再在检测到障碍物时停下，这样你就能看到传感器是怎么帮助机器人“观察”世界的。',
    ].join('\n\n');
  }

  if (text.includes('program') || text.includes('code') || text.includes('编程')) {
    return [
      '编程时可以先想三个问题：什么时候开始？要做什么？什么时候停止？',
      '比赛里很好用的结构是“前进 -> 检测 -> 判断 -> 调整”。',
      '如果你愿意，可以先从一个超短流程开始：前进 2 秒，转弯，停下，再慢慢加上传感器判断。',
    ].join('\n\n');
  }
  ...
  return [
    '这是一个很棒的问题。',
    '机器人比赛通常可以从三个方向来学：机械结构、程序逻辑、现场策略。',
    '你可以先告诉我你最想学哪一个，比如“怎么让机器人走直线”或者“怎样准备比赛任务”，我会一步一步陪你学。',
  ].join('\n\n');
}
...
export async function getRobotCoachReply(messages: ChatMessage[]): Promise<LlmResponse> {
  try {
    ...
    return {
      reply: [
        '我刚刚没有顺利连上在线教练，所以先用本地训练模式继续陪你学习。',
        `小提示：${message}`,
