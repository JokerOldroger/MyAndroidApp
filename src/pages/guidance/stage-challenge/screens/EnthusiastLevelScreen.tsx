...
import { RootStackParamList, navigateToMainTab } from '../../shared/navigation/navigationTypes';
import { enthusiastLevels } from '../data/enthusiastLevels';

type EnthusiastLevelScreenProps = NativeStackScreenProps<RootStackParamList, 'EnthusiastLevel'>;
...
const moduleLibrary: ModuleDefinition[] = [
  {
    ...
    title: '设置轮速',
    shortLabel: '轮速',
    description: '把左右轮都设置成相同速度，让叮咚更容易走直线。',
    color: '#67D39A',
    promptLine: '把叮咚左右轮速都设置为 40，保持相同输出，让叮咚稳定直线前进。',
  },
  {
    id: 'distance',
    title: '设置直线距离',
    shortLabel: '距离',
    description: '告诉叮咚要向前直线移动多远，比如 120 厘米。',
    color: '#7FDFFF',
    promptLine: '让叮咚沿直线移动 120 厘米，到达目标点后停止。',
  },
  {
    id: 'obstacle',
    title: '障碍判断',
    shortLabel: '避障',
    description: '当前方距离小于 20 厘米时，让叮咚停下并绕开障碍。',
    color: '#FFB86E',
    promptLine: '如果叮咚前方障碍距离小于 20 厘米，就立即停止，轻微右转绕开，再回到原来的直线路线。',
  },
  {
    id: 'loop',
    title: '往返循环',
    shortLabel: '循环',
    description: '完成一次前进后返回起点，并重复 3 轮，用来做叮咚往返跑训练。',
    color: '#D2A2FF',
    promptLine: '把叮咚前进和返回起点作为一轮，连续执行 3 轮往返跑训练。',
  },
];
...
export default function EnthusiastLevelScreen({ navigation, route }: EnthusiastLevelScreenProps) {
  const level = enthusiastLevels.find((item) => item.id === route.params.levelId) ?? enthusiastLevels[1];
  const [programBlocks, setProgramBlocks] = useState<ModuleDefinition[]>([]);
  const [feedback, setFeedback] = useState(`这是 ${level.currentTag}，先把适合叮咚的基础模块拼好，再挑战更复杂的动作。`);
  ...
  const aiPromptPreview = useMemo(() => {
    if (programBlocks.length === 0) {
      return '下发给叮咚的运动执行指令会显示在这里。先选择模块，再组合逻辑。';
    }
  ...
  function addModule(moduleItem: ModuleDefinition) {
    ...
    setHasPassed(false);
    setFeedback(`已加入模块：${moduleItem.title}。继续把叮咚逻辑拼完整吧。`);
  }
  ...
  function resetProgram() {
    setProgramBlocks([]);
    setHasPassed(false);
    setFeedback('程序区已清空。重新开始组合新的叮咚控制逻辑吧。');
  }

  function runProgram() {
    const missing = level.modules.filter((item) => !selectedIds.includes(item));
    ...
    if (speedIndex > distanceIndex) {
      setHasPassed(false);
      setFeedback('建议先设置轮速，再设置直线移动距离，这样叮咚会先知道怎么稳定驱动轮子。');
      return;
    }

    if (loopIndex !== -1 && loopIndex < distanceIndex) {
      setHasPassed(false);
      setFeedback('往返循环应该放在基础运动逻辑之后，让叮咚先学会直行，再重复执行。');
      return;
    }

    setHasPassed(true);
    setFeedback(`闯关成功。${level.currentTag} 已完成，你可以返回地图继续下一关，并把方案继续下发给叮咚。`);
  }
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={[styles.heroCard, { backgroundColor: level.themeColor }]}>
            <View style={styles.heroCopy}>
              <Text style={styles.heroBadge}>DINGDONG CHALLENGE · {level.currentTag}</Text>
              <Text style={styles.heroTitle}>{level.title}</Text>
              <Text style={styles.heroSubtitle}>叮咚爱好者世界 · 现在进行到 {level.currentTag}</Text>
            </View>
          ...
          <View style={styles.levelBanner}>
            <View>
              <Text style={styles.levelBannerLabel}>当前关卡</Text>
              <Text style={styles.levelBannerTitle}>{level.currentTag}</Text>
            </View>
            <Pressable
              style={styles.backButton}
              onPress={() => navigation.goBack()}
              accessibilityRole="button"
              accessibilityLabel="返回爱好者地图"
            >
              <Text style={styles.backButtonText}>返回地图</Text>
            </Pressable>
          </View>

          <View style={styles.missionCard}>
            <Text style={styles.sectionTitle}>本关目标</Text>
            <Text style={styles.missionBody}>{level.missionBody}</Text>
          </View>
          ...
          <View style={styles.moduleGrid}>
            {moduleLibrary.map((moduleItem) => (
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel={`添加模块 ${moduleItem.title}`}
              >
          ...
          <View style={styles.actionRow}>
            <Pressable style={styles.primaryButton} onPress={runProgram} accessibilityRole="button" accessibilityLabel="运行程序">
              <Text style={styles.primaryButtonText}>运行程序</Text>
            </Pressable>
            <Pressable style={styles.secondaryButton} onPress={resetProgram} accessibilityRole="button" accessibilityLabel="重置程序">
              <Text style={styles.secondaryButtonText}>重置程序</Text>
            </Pressable>
            <Pressable style={styles.secondaryButton} onPress={() => navigateToMainTab(navigation, 'AI')} accessibilityRole="button" accessibilityLabel="问叮咚教练">
              <Text style={styles.secondaryButtonText}>问叮咚教练</Text>
            </Pressable>
          </View>
          ...
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>给叮咚的运动指令 Prompt</Text>
            ...
            <>
              ...
              <View style={styles.explainCard}>
                ...
                <Text style={styles.explainText}>
                  1. 先设置左右轮速一致，这样叮咚更容易保持直线方向。{'\n'}
                  2. 再给出直线移动距离，让叮咚控制器知道要前进多远。{'\n'}
                  3. 使用 if 判断障碍距离是否小于 20 厘米，如果太近就先停下，再转向绕开。{'\n'}
                  4. 使用 for 循环把前进和返回过程重复 3 轮，形成叮咚往返跑训练。{'\n'}
